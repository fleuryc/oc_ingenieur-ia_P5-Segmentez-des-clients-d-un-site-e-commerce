...
dataset: ## Download and extract dataset from Kaggle
	...
	@if [ ! -f "data/raw/olist_customers_dataset.csv" ] \
		|| [ ! -f "data/raw/olist_geolocation_dataset.csv" ] \
		|| [ ! -f "data/raw/olist_order_items_dataset.csv" ] \
		|| [ ! -f "data/raw/olist_order_payments_dataset.csv" ] \
		|| [ ! -f "data/raw/olist_order_reviews_dataset.csv" ] \
		|| [ ! -f "data/raw/olist_orders_dataset.csv" ] \
		|| [ ! -f "data/raw/olist_products_dataset.csv" ] \
		|| [ ! -f "data/raw/olist_sellers_dataset.csv" ] \
		|| [ ! -f "data/raw/product_category_name_translation.csv" ] ; \
	then \
		echo "Downloading data..." ; \
		kaggle datasets download -d olistbr/brazilian-ecommerce -p data/raw/ ; \
		echo "Unzipping data..."; \
		unzip -u -q data/raw/brazilian-ecommerce.zip -d data/raw/ ; \
		echo "Done."; \
	else \
